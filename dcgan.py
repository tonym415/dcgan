...
def discriminator(x, reuse=False):
    with tf.variable_scope('Discriminator', reuse=reuse):
        x = tflearn.conv_2d(x, 64, 5, 1, activation='tanh')
...
gp = GANPipe()
if not gp.TRAIN_SET_FILE_LIST:
    gp.get_train_test_file_list()
    proc_path = gp.PREPROCESSED_DATA_FOLDER
    for f_in in gp.TRAIN_SET_FILE_LIST:
        full_path = proc_path + f_in
        ...
        X = np.reshape(X, newshape=[-1, 250, 250, 1])
        ...
        input_disc_real = tflearn.input_data(shape=[None, 250, 250, 1], name='input_disc_real')
